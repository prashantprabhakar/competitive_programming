...
function largestDivisibleSubset(nums) {
    ...
    // DP matrix to store maximum number of LDS at given index
    const noOfLds = Array(nums.length).fill(1);
    // index at which no of largesr divisible subset is greatest
    let maxIndex = 0

    for(let i=1; i<=nums.length-1; i++) {
        for(let j=i-1; j>=0; j--) {
            if(nums[i] % nums[j] === 0) {
                noOfLds[i] = Math.max(noOfLds[i], noOfLds[j] + 1);
                if(noOfLds[i] > noOfLds[maxIndex]  ) {
                    maxIndex = i;
                }
            }
        }
    ...
    const result = [];
    let maxSize = noOfLds[maxIndex];
    let num = nums[maxIndex]
    for(let i=maxIndex; i>=0; i--) {
        if(
            noOfLds[i] === maxSize &&
            // This condition is needed beacuse there can be more than one possible solutions
            num % nums[i] === 0) {
            result.push(nums[i]);
            num = nums[i]
            maxSize--;
...
function largestDivisibleSubset2(nums) {
    nums.sort((a, b) => a - b);
    const n = nums.length;
